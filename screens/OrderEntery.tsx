...
const OrderEntry = ({ navigation }: any) => {
  ...
  const onSearch = async (value) => {
    axios
      .post("https://hgsonsapp.hgsons.in/master/search_order.php", {
        Search: value,
        Token: await getStoreValue("token"),
      })
      .then((res) => {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#FFFAF0" }}>
      ...
      <View style={styles.titleMain}>
        ...
        >
          <TouchableOpacity
            onPress={() => {
              ...
              setShowCatalogOrder(false);
            }}
          >
            <Text style={styles.opsText}>{"Orders"}</Text>
          </TouchableOpacity>
          {showSingleOrder && (
            <View
              style={{
        ...
        <TouchableOpacity
          onPress={() => {
            setShowCatalogOrder(true);
            setShowSingleOrder(false);
          }}
        >
          <Text style={styles.opsText}>{"Catalogs"}</Text>
          {showCatalogOrder && (
            <View
              style={{
                height: 5,
